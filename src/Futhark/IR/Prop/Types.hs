...
elemToType (ElemAcc r v) = Acc r v
...
modifyArrayShape _ (Acc r v)   = Acc r v
...
arrayOf (Acc r v) shape u
  | 0 <- shapeRank shape = Acc r v
  | otherwise = Array (ElemAcc r v) shape u
arrayOf Mem{} _ _ =
  error "arrayOf Mem"
...
hasStaticShape (Acc r v) = Just $ Acc r v
